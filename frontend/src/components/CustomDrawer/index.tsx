...
import { CustomDrawerContext } from '../../contexts/useDrawer';
import "./index.scss"
...
export default function CustomDrawer(props: any) {
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      <CssBaseline />
      <AppBar position="fixed" className='drawer' open={open}>
        <Toolbar>
          <IconButton
            color="inherit"
      ...
      <Drawer variant="permanent" className='drawer-bar' open={open}>
        ...
        <Divider />
        <List className='drawer-bar'>
            { userId != 0
                ? navData.map((data, index) => (
                    <ListItem className='list-item' key={data.text} disablePadding sx={{ display: 'block' }}>
                        <Link to={data.link}>
                            <ListItemButton
                            sx={{
                                minHeight: 48,
